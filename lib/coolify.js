...
async function provisionInstance({ customerId, subscriptionId, email, company, plan }) {
  ...
  // 3. Assign public subdomain if COOLIFY_DOMAIN_TEMPLATE is configured
  let appUrl = null;
  if (DOMAIN_TPL) {
    appUrl = DOMAIN_TPL.replace('{slug}', slug);
    await api('PATCH', `/applications/${app.uuid}`, { fqdn: appUrl });
  }
